/*
* THIS IS A SLIGHTLY REVISITED VERSION OF THE
*
*           ftp-client/lib/client.js
*
* as it causes some problems when trying to do get
* use it at your own risk
*
* after doing npm install simply replace ftp-client/lib/client.js
* with this one
*
* enjoy!!!!!
*
...
var fs = require('fs'),
    ...
    loggingType = 'debug',
    loggingLevel = loggingLevels.indexOf(loggingType),
    log = function (msg, lvl) {
        if (loggingLevels.indexOf(lvl) <= loggingLevel) {
            console.log(msg);
        }
    };
        ...
        ftp.list(task.src, function (err, list) {
            ...
            if (list && list.length >= 1) {
                ...
                _.each(list.splice(0, list.length), function (file) {
                    var patternToFind = new RegExp(options.fileNamePattern, 'i');
                    if (file.name !== '.' && file.name !== '..' &&  patternToFind.test(file.name)) {
                        log(file.name, 'debug');
